...
internal class LicenseCommand: CliktCommand(help = "Adds a LICENSE file to the current project") {
  ...
  override fun run() {
    val license = license ?: promptChoices("License", choices = SupportedLicense.entries.map { it.name }).toSupportedLicense()
    val copyright = copyright ?: promptInput("Copyright holder")
    Path(".").apply {
